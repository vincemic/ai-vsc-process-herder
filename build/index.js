...
class VSCodeProcessHerderServer {
    ...
    constructor() {
        ...
        this.recoveryManager = new ProcessRecoveryManager(this.processManager, this.healthMonitor);
    ...
    setupIntegrations() {
        ...
        this.processManager.on("processRegistered", (pid, metadata) => {
            this.healthMonitor.startMonitoring(pid, metadata);
            this.stateManager.registerProcessState(pid, metadata);
            this.loggingManager.info("process-lifecycle", `Process registered for monitoring`, { pid, name: metadata.name });
        });
        // Setup recovery notifications
        this.recoveryManager.on("recovery-started", (attempt) => {
            this.loggingManager.warn("recovery", `Recovery started for ${attempt.processName}`, attempt);
        });
        this.recoveryManager.on("recovery-success", (attempt) => {
            this.loggingManager.info("recovery", `Recovery successful for ${attempt.processName}`, attempt);
        });
        this.recoveryManager.on("recovery-failed", (attempt) => {
            this.loggingManager.error("recovery", `Recovery failed for ${attempt.processName}`, attempt);
        });
        // Setup health monitoring alerts
        this.healthMonitor.on("critical", (healthResult) => {
            this.loggingManager.critical("health", `Critical health issue detected`, healthResult);
        });
    }
    setupTools() {
        // Tool: List available VS Code tasks
        // Test run lifecycle tools
        this.server.registerTool("start-test-run", {
            ...
            try {
                ...
                const normalize = (r) => {
                    if (!r)
                        return undefined;
                    if (r.type === "port")
                        return { type: "port", value: Number(r.value), timeoutMs: r.timeoutMs, intervalMs: r.intervalMs };
                    if (r.type === "http")
                        return { type: "http", value: String(r.value), timeoutMs: r.timeoutMs, intervalMs: r.intervalMs };
                    if (r.type === "log")
                        return { type: "log", value: r.value instanceof RegExp ? r.value : String(r.value), timeoutMs: r.timeoutMs };
                    return undefined;
                };
        ...
        this.server.registerTool("get-test-run-status", {
        ...
        this.server.registerTool("abort-test-run", {
        ...
        this.server.registerTool("list-test-runs", {
        ...
        this.server.registerTool("list-tasks", {
        ...
        this.server.registerTool("start-task", {
        ...
        this.server.registerTool("stop-process", {
            title: "Stop Process",
        ...
        this.server.registerTool("restart-process", {
        ...
        this.server.registerTool("list-processes", {
        ...
        this.server.registerTool("start-process", {
            title: "Start Arbitrary Process",
            description: "Start a process by command with optional role classification and metadata.",
            inputSchema: {
            ...
            try {
                ...
                const result = await this.processManager.startProcess(command, args, { cwd, name, role: inferredRole, tags, singleton, readiness: readinessConfig });
                return {
                    content: [
                        {
                            type: "text",
        ...
        this.server.registerTool("get-process-status", {
            ...
            try {
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                healthHistory,
                                processState,
                                lastHealthCheck: healthHistory[healthHistory.length - 1] || null,
                            }, null, 2),
                        },
                    ],
                };
                ...
                this.loggingManager.error("process-status", "Failed to get process status", {
                    error: error instanceof Error ? error.message : String(error),
                    processId,
                    processName,
                });
        ...
        this.server.registerTool("detect-project-type", {
        ...
        this.server.registerTool("get-health-summary", {
            ...
            try {
                ...
                const processesNeedingAttention = this.healthMonitor.getProcessesNeedingAttention();
                ...
                this.loggingManager.error("health-summary", "Failed to get health summary", { error: error instanceof Error ? error.message : String(error) });
                return {
                    content: [
                        {
                            type: "text",
        ...
        this.server.registerTool("check-port-status", {
        ...
        this.server.registerTool("stop-processes-by-port", {
        ...
        this.server.registerTool("find-processes-by-port", {
        ...
        this.server.registerTool("list-active-ports", {
        ...
        this.server.registerTool("get-process-logs", {
            title: "Get Process Logs",
            description: "Get filtered logs for processes with advanced filtering options",
            inputSchema: {
            ...
            try {
                ...
                this.server.registerTool("detect-port-conflicts", {
        ...
        this.server.registerTool("get-process-metrics", {
            ...
            try {
                ...
                if (metricName) {
                    ...
                    const stats = metric
                        ? this.loggingManager.getMetricStats(metricName, timeRange
                            ? {
                ...
                this.loggingManager.error("get-metrics", "Failed to get process metrics", { error: error instanceof Error ? error.message : String(error) });
                return {
                    content: [
                        {
                            type: "text",
        ...
        this.server.registerTool("configure-recovery", {
        ...
        }, async ({ processName, enabled = true, maxRestarts = 3, strategy = "restart", }) => {
            try {
                // This is a simplified configuration - in practice would be more complex
                this.loggingManager.info("recovery-config", `Recovery configured for ${processName}`, {
                    processName,
                    enabled,
                    maxRestarts,
                    strategy,
                ...
                this.loggingManager.error("configure-recovery", "Failed to configure recovery", { error: error instanceof Error ? error.message : String(error) });
                return {
                    content: [
                        {
                            type: "text",
        ...
        this.server.registerTool("export-diagnostics", {
        ...
        }, async ({ includeState = true, includeLogs = true, includeMetrics = true, timeRange, }) => {
            try {
                const exportData = {
                    exportTime: new Date(),
                    systemInfo: {
                ...
                if (includeLogs) {
                    const logFilter = timeRange
                        ? {
                            startTime: new Date(Date.now() - timeRange.hours * 60 * 60 * 1000),
                        }
                        : undefined;
                    exportData.logs = {
                ...
                this.loggingManager.error("export-diagnostics", "Failed to export diagnostics", { error: error instanceof Error ? error.message : String(error) });
                return {
                    content: [
                        {
                            type: "text",
        ...
        this.server.registerTool("get-vscode-status", {
    ...
    setupResources() {
        ...
        this.server.registerResource("tasks-config", "file://.vscode/tasks.json", {
        ...
        this.server.registerResource("process-summary", "process://summary", {
            ...
            try {
                ...
                const stats = {
                    ...
                    byRole: summary.reduce((acc, s) => { acc[s.role || 'unassigned'] = (acc[s.role || 'unassigned'] || 0) + 1; return acc; }, {}),
        ...
        this.server.registerResource("process-logs", "logs://process-herder/all-processes", {
            title: "Process Logs",
    ...
    setupPrompts() {
        ...
        this.server.registerPrompt("dev-workflow-help", {
...
Available tasks: ${tasks.map((t) => t.label).join(", ")}
Running processes: ${processes.length} (${processes.filter((p) => p.metadata).length} managed)
...
Please suggest the most relevant tasks for my project type and current state.`,
        ...
        this.server.registerPrompt("process-summary", {
