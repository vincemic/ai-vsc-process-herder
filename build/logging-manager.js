...
export class LoggingManager extends EventEmitter {
     ...
     */
    log(level, category, message, metadata, processId) {
     ...
     */
    debug(category, message, metadata, processId) {
        this.log("debug", category, message, metadata, processId);
    }
    info(category, message, metadata, processId) {
        this.log("info", category, message, metadata, processId);
    }
    warn(category, message, metadata, processId) {
        this.log("warn", category, message, metadata, processId);
    }
    error(category, message, metadata, processId) {
        this.log("error", category, message, metadata, processId);
    }
    critical(category, message, metadata, processId) {
     ...
     */
    getLogs(filter, limit = 1000) {
        let filteredLogs = [...this.logs];
        if (filter) {
            if (filter.level) {
                const levelPriority = this.getLevelPriority(filter.level);
                filteredLogs = filteredLogs.filter((log) => this.getLevelPriority(log.level) >= levelPriority);
            }
            if (filter.category) {
                filteredLogs = filteredLogs.filter((log) => log.category.toLowerCase().includes(filter.category.toLowerCase()));
            }
            if (filter.processId) {
                filteredLogs = filteredLogs.filter((log) => log.processId === filter.processId);
            }
            if (filter.startTime) {
    ...
    clearMetrics(olderThan) {
        ...
        if (olderThan) {
            for (const metric of this.metrics.values()) {
                ...
                metric.points = metric.points.filter((point) => point.timestamp >= olderThan);
        ...
        try {
            ...
            if (data.metrics) {
                for (const metric of data.metrics) {
                    ...
                    metric.points = metric.points.map((p) => ({
                        ...p,
...
//# sourceMappingURL=logging-manager.js.map