...
export class ProjectDetector {
     ...
     */
    async detectProject(workspaceRoot) {
        ...
        const topType = Object.entries(typeConfidence).sort(([, a], [, b]) => b - a)[0];
        if (topType && topType[1] > 0) {
            projectType = topType[0];
            confidence = topType[1];
        }
        // Generate suggested tasks based on detected project type
        const suggestedTasks = this.generateSuggestedTasks(projectType, frameworks, packageManagers, root);
        return {
            type: projectType,
            frameworks,
            packageManagers,
     ...
     */
    async checkNodeProject(root, findings, frameworks, packageManagers) {
     ...
     */
    detectNodeFrameworks(dependencies, frameworks, findings) {
        const frameworkMap = {
            react: "React",
            vue: "Vue.js",
            angular: "Angular",
     ...
     */
    async checkPythonProject(root, findings, frameworks, packageManagers) {
        const pythonFiles = [
            "requirements.txt",
            "setup.py",
            "pyproject.toml",
     ...
     */
    detectPythonFrameworks(requirements, frameworks, findings) {
        const frameworkMap = {
            django: "Django",
            flask: "Flask",
            fastapi: "FastAPI",
     ...
     */
    async checkRustProject(root, findings, frameworks, packageManagers) {
     ...
     */
    async checkGoProject(root, findings, frameworks, packageManagers) {
     ...
     */
    async checkJavaProject(root, findings, frameworks, packageManagers) {
     ...
     */
    async checkDotNetProject(root, findings, frameworks, packageManagers) {
     ...
     */
    async checkWebProject(root, findings, frameworks) {
        ...
        if (this.hasFilesWithExtension(root, ".css") ||
     ...
     */
    hasFilesWithExtension(root, extension) {
        try {
     ...
     */
    hasFilesWithPattern(root, pattern) {
        try {
            const files = fs.readdirSync(root);
     ...
     */
    generateSuggestedTasks(projectType, frameworks, packageManagers, root) {
        const tasks = [];
        switch (projectType) {
            case "javascript":
