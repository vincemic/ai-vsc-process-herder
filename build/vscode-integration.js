...
export class VSCodeIntegration {
     ...
     */
    async parseWorkspaceFile(workspaceFile) {
        try {
     ...
     */
    async getWorkspaceSettings(workspaceRoot) {
     ...
     */
    async updateWorkspaceSettings(settings, workspaceRoot) {
        const root = workspaceRoot || process.cwd();
     ...
     */
    async getLaunchConfig(workspaceRoot) {
        const root = workspaceRoot || process.cwd();
     ...
     */
    async updateLaunchConfig(config, workspaceRoot) {
        const root = workspaceRoot || process.cwd();
        const vscodeDir = path.join(root, ".vscode");
     ...
     */
    async getRecommendedExtensions(workspaceRoot) {
        const root = workspaceRoot || process.cwd();
     ...
     */
    async updateRecommendedExtensions(recommendations, workspaceRoot) {
        const root = workspaceRoot || process.cwd();
        const vscodeDir = path.join(root, ".vscode");
