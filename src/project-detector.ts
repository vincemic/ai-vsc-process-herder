...
export class ProjectDetector {
  ...
  private async checkWebProject(
    ...
    if (this.hasFilesWithExtension(root, '.css') || this.hasFilesWithExtension(root, '.scss') || this.hasFilesWithExtension(root, '.sass')) {
      findings.push({
        file: '*.css/*.scss/*.sass',
        type: 'source',
   ...
   */
  private calculateTypeConfidence(findings: ProjectFinding[]): Record<string, number> {
    ...
    for (const finding of findings) {
      ...
      if (finding.file.includes('package.json') || finding.file.includes('node_modules') || 
          finding.file.includes('.js') || finding.file.includes('.ts')) {
        projectType = 'javascript';
      } else if (finding.file.includes('.py') || finding.file.includes('requirements.txt') || 
                 finding.file.includes('setup.py')) {
        projectType = 'python';
      } else if (finding.file.includes('Cargo.toml') || finding.file.includes('.rs')) {
        projectType = 'rust';
      } else if (finding.file.includes('go.mod') || finding.file.includes('.go')) {
        projectType = 'go';
      } else if (finding.file.includes('.java') || finding.file.includes('pom.xml') || 
                 finding.file.includes('build.gradle')) {
        projectType = 'java';
      } else if (finding.file.includes('.cs') || finding.file.includes('.csproj') || 
                 finding.file.includes('.sln')) {
        projectType = 'dotnet';
      } else if (finding.file.includes('.html') || finding.file.includes('.css')) {
        projectType = 'web';
      }

      typeScores[projectType] = (typeScores[projectType] || 0) + finding.confidence;
    }
