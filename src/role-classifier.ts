export type InferredRole = "frontend" | "backend" | "test" | "e2e" | "utility";

interface RoleHeuristic {
  role: InferredRole;
  patterns: RegExp[];
}

const heuristics: RoleHeuristic[] = [
...
export function inferRole(nameOrCommand: string): InferredRole {
  const lowered = nameOrCommand.toLowerCase();
  for (const h of heuristics) {
    if (h.patterns.some((p) => p.test(lowered))) return h.role;
  }
  return "utility";
}
