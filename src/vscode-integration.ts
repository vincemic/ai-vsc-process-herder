...
export class VSCodeIntegration {
  ...
  async getWorkspaceSettings(
    ...
    try {
      ...
      throw new Error(
        `Failed to parse VS Code settings: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async getLaunchConfig(workspaceRoot?: string): Promise<any> {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse launch.json: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
   ...
   */
  async updateLaunchConfig(config: any, workspaceRoot?: string): Promise<void> {
    const root = workspaceRoot || process.cwd();
    const vscodeDir = path.join(root, ".vscode");
